...
class Trainer():
    ...
    def build_model(self):
        if self.args.model == "DCGAN":
            ...
            from DCGAN_1_adjusted import Generator, Discriminator
        elif self.args.model == "standard_CNN":
            from standardCNN_adjusted import Generator, Discriminator
        # Initialize weights
        def weights_init(m):
        ...
        if self.args.adv_loss == "SGAN":
            self.D = Discriminator(self.image_height, self.image_width, self.embedding_dim,
                               self.args.spectral_D, self.args.batch_norm_D, self.args.bias_D, sigmoid=True).to(self.device)
        else:
            self.D = Discriminator(self.image_height, self.image_width, self.embedding_dim,
        ...
        for epoch in range(start, self.num_epoch): 
            ...
            for step in range(0, steps_per_epoch):
                ...
                if self.args.adv_loss == 'WGAN-GP':
                    ...
                    alpha = torch.rand(current_batch_size, 1, 1, 1).to(self.device).expand_as(real_images)
                    interpolated = Variable(alpha * current_batch_size + (1 - alpha) * fake_images.data, requires_grad=True)
                    out = self.D(interpolated, self.roi.unsqueeze(0).repeat(current_batch_size,1,1,1), fake_values)
